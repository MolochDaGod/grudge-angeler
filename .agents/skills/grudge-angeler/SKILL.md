...
### Grudge ID System (Architecture Plan)

**Authentication Method:** Replit Auth (OpenID Connect) or Puter login
- Supports Google, GitHub, Apple, X, email/password
- Server-side sessions stored in PostgreSQL
- User gets a unique `sub` claim = their **Grudge ID**
- No custom login forms needed — redirect to `/api/login`

**Auth Flow:**
```
Landing/Advert Page (logged out) → Login Button (/api/login) → Auth Provider
→ Callback → Server creates/updates user → Redirect to game
→ Character Select (faction + name) → Game with server-side wallet
```

**Server-Side Integration Files (auto-generated by Replit Auth):**
```
server/replit_integrations/auth/index.ts     - setupAuth, isAuthenticated, registerAuthRoutes
server/replit_integrations/auth/replitAuth.ts - OIDC passport setup
server/replit_integrations/auth/storage.ts    - getUser, upsertUser
server/replit_integrations/auth/routes.ts     - /api/auth/user endpoint
shared/models/auth.ts                         - Drizzle schema for users + sessions
client/src/hooks/use-auth.ts                  - React useAuth() hook
client/src/lib/auth-utils.ts                  - isUnauthorizedError helper
```

**Auth Routes:**
```
/api/login       - Start login flow (redirect, not a form)
/api/logout      - Start logout flow
/api/auth/user   - Get current user (sub, email, name, profile_image_url)
```

**Wiring Steps:**
1. Install Replit Auth integration (adds files above)
2. In server routes: `await setupAuth(app)` + `registerAuthRoutes(app)` BEFORE other routes
3. Export `* from "./models/auth"` in shared/schema.ts
4. Run `npm run db:push` to create users + sessions tables
5. Protect game API routes with `isAuthenticated` middleware
6. Use `useAuth()` hook in React to check login state

**Database Schema (New Tables):**
```typescript
// shared/models/auth.ts (auto-generated)
users: { id, username, email, firstName, lastName, profileImageUrl, createdAt, updatedAt }
sessions: { sid (PK), sess (json), expire (timestamp) }

// New table needed for wallets:
wallets: { id, userId (FK→users.id, unique), gbux (integer, default 0), headOfLegends (integer, default 0) }

// New table for save state:
game_saves: { id, userId (FK→users.id, unique), faction, playerName, fishCollection (json), rodLevel, totalCatches, ... }
```

**Wallet API Routes (to build):**
```
GET  /api/wallet           - Get current user's wallet balances (requires auth)
POST /api/wallet/earn      - Add gbux from fish sale (server-validated)
POST /api/wallet/spend     - Spend gbux/HoL on items (server-validated)
GET  /api/save             - Load game save for current user
POST /api/save             - Save game state for current user
```

**Client-Side Changes:**
- Landing page: game promo/advert with LOGIN button → `/api/login`
- `useAuth()` in App.tsx to gate game access
- On login: load wallet + save state from server
- On fish sell: POST to `/api/wallet/earn` instead of local gbux
- Leaderboard entries auto-linked to Grudge ID

**User Claims Available:**
```typescript
req.user.claims.sub            // Grudge ID (stable unique string)
req.user.claims.email          // Email (nullable)
req.user.claims.first_name     // First name (nullable)
req.user.claims.last_name      // Last name (nullable)
req.user.claims.profile_image_url // Profile picture URL
```

**Key Rules:**
- SESSION_SECRET is already available as an environment secret
- Never create custom login forms — use `/api/login` redirect
- Users + sessions tables are mandatory — never drop them
- Keep `default` config on user `id` column
- Do NOT modify auth module files unless absolutely necessary
