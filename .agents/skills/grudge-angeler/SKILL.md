...
**Authentication Method:** Discord OAuth2
- Users log in via Discord account
- Server-side cookie sessions
- User gets a unique Discord ID = their **Grudge ID**
- Production URL: `https://grudge-angeler.vercel.app`
...
GET  /api/auth/callback   - Discord callback, sets session cookie
GET  /api/auth/me          - Get current user (discordId, username, avatar)
POST /api/auth/logout      - Clear session cookie
```

**Server-Side Integration:**
- Discord OAuth handled in `server/routes.ts` (getDiscordTokens, getDiscordUser)
- In-memory sessions (Map-based) with cookie `ga_session`
- Environment vars: `DISCORD_CLIENT_ID`, `DISCORD_CLIENT_SECRET`
